...
final class OrderController extends AbstractController
{
    ...
    {
        $client = $this->getUser();
        if ($order->getClient() !== $client && !$this->isGranted('ROLE_ADMIN')) {
            throw $this->createAccessDeniedException('У вас нет прав для просмотра этого заказа');
        }
    ...
    public function edit(Request $request, Order $order, EntityManagerInterface $entityManager, SluggerInterface $slugger): Response
    {
        ...
        $client = $this->getUser();
        if ($order->getClient() !== $client && !$this->isGranted('ROLE_ADMIN')) {
            throw $this->createAccessDeniedException('У вас нет прав для редактирования этого заказа');
        }
    ...
    public function deleteFile(Request $request, Order $order, int $fileId, EntityManagerInterface $entityManager): Response
    {
        ...
        $client = $this->getUser();
        if ($order->getClient() !== $client && !$this->isGranted('ROLE_ADMIN')) {
            throw $this->createAccessDeniedException('У вас нет прав для удаления файла');
        }
    ...
    public function delete(Request $request, Order $order, EntityManagerInterface $entityManager): Response
    {
        $client = $this->getUser();
        if ($order->getClient() !== $client && !$this->isGranted('ROLE_ADMIN')) {
            throw $this->createAccessDeniedException('У вас нет прав для удаления этого заказа');
        }
    ...
    public function downloadFile(Order $order, int $fileId, EntityManagerInterface $entityManager): Response
    {
        $client = $this->getUser();
        if ($order->getClient() !== $client && !$this->isGranted('ROLE_ADMIN')) {
            throw $this->createAccessDeniedException('У вас нет прав для скачивания этого файла');
        }
